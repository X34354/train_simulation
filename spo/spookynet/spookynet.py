...
class SpookyNet(nn.Module):
    ...
    for short-range repulsion, electrostatics and dispersion.
    ...
    are not used, no changes are necessary.
    ...
    Arguments:
        ...
        num_residual_pre_local_i (int):
            ...
            (central atoms) before computing the interaction (per module).
        num_residual_pre_local_j (int):
            Number of residual blocks applied to atomic features in j branch
            (neighbouring atoms) before computing the interaction (per module).
        num_residual_post_local (int):
            Number of residual blocks applied to interaction features (per
            module).
    ...
    def energy(
        ...
        """
        print(Z.shape,Q.shape,S.shape,R.shape)
        if batch_seg is None:  # assume a single batch
            batch_seg = Z.new_zeros(Z.size(0))
